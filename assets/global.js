...
class VariantSelects extends HTMLElement {
  ...
  updateMasterId() {
    ...
    if (this.currentVariant) {
      const colorName = this.currentVariant.title;
      const isAvailable = this.currentVariant.available;
      document.getElementById("js-current-color").innerHTML = colorName;
      const currentStockElement = document.getElementById("js-current-stock");
      if (currentStockElement) {
        currentStockElement.innerHTML = isAvailable
          ? "In Stock"
          : "Out of Stock";
      }
    }
  ...
  updateURL() {
    ...
    const customizerImageContainers =
      document.querySelectorAll(".customizer__sides");

    [...customizerImageContainers].forEach(container => {
      const varId = container.getAttribute("data-variantid");

      Number(varId) === Number(this.currentVariant.id)
...
function accordionList(accordionArray) {
  const accordionItem = document.querySelectorAll(accordionArray);

  accordionItem.forEach((btn, btnIndex, btns) => {
    btn.addEventListener("click", () => {
      btns.forEach((item, itemIndex) => {
        if (item.classList.contains("is-open") && btnIndex != itemIndex) {
...
accordionList(".js-has-mega-menu--mobile");
